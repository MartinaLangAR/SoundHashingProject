...
SimpleFaceDetector::SimpleFaceDetector() :
    ...
    // and MODEL_WEIGHTS_FILE are passed in via cmake
        network_ = cv::dnn::readNetFromCaffe("deploy.prototxt",
                "res10_300x300_ssd_iter_140000_fp16.caffemodel");
     if (network_.empty()) {
        std::ostringstream ss;
        ss << "Failed to load network with the following settings:\n"
           << "Configuration: " + std::string("deploy.prototxt") + "\n"
           << "Binary: " + std::string("res10_300x300_ssd_iter_140000_fp16.caffemodel") + "\n";
        throw std::invalid_argument(ss.str());
    }
}

std::vector<cv::Rect> SimpleFaceDetector::detect_face_rectangles(const cv::Mat &frame) {
